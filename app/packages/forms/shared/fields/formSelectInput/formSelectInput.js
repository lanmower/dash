...
if(Meteor.isClient) {
  ...
  Template.afFormList.onCreated(function () {
    ...
    self.subscribe("form", data.atts.form, {
      onReady: function () {
        self.subscribe('formSearch', data.atts.form,"",{
          onReady:function() {
          }
        });
      },
...
Fields.schemas.formSelectInput = function(data) {
  ...
  output[name] = {
        ...
        autoform: {
          ...
          afFieldInput: {
          options: function () {
            var form = Forms.findOne({_id:data.form});
            var collection = getCollection(data.form);
            if(collection) {
              var list = [];
              _.each(collection.find().fetch(), function(doc) {
                list.push({label:doc.label, value:doc._id});
              });
