...
if (Meteor.isClient) {
  ...
  Template.afLeaveInput.onCreated(function() {
    ...
    var field = Fields.findOne({
      parent: Router.current().params.form,
      name: data.name
    });
                ...
                template.subscribe('approvals-form', Router.current().params.form, {
                  onReady: function() {
                    ...
                    _.each(docs, function(doc) {
                      ...
                      if (totalHours >= 0) toSet = {
                        label: doc.title + ": " + totalHours + " hours remaining.",
                        value: doc._id
                      };
                      else toSet = {
                        label: doc.title + ": 0 hours remaining. " + num + " overbooked!",
                        value: doc._id
                      };
                      list.push(toSet);
                    });
